...
import { MenuData, MenuItem } from "@/shared/types/api";
...
const StoreButton = ({
  ...
  return (
    <a
      ...
      className="flex items-center gap-3 bg-[#262626] text-white px-5 py-2.5 rounded-[14px] hover:bg-black active:scale-95 transition-all min-w-37.5"
    >
      {isApple ? (
    ...
    <footer className="bg-[#F9F9F9] pt-10 md:pt-16 pb-22 lg:pb-10 border-t border-gray-200 text-[#4B4B4B] font-rubik">
      <div className="max-w-350 mx-auto px-5 md:px-8">
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-8 mb-8 lg:mb-16">
          ...
          <div className="lg:w-2/3 grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-10 order-2 lg:order-0">
                ...
                <ul className="flex flex-col gap-3.5">
                  {section.links.map((link, lIdx) => (
                    <li key={lIdx}>
                      <Link
                        href={link.href}
                        className="text-[13px] md:text-xs font-bold md:uppercase text-[#4B4B4B] hover:text-[#FFD600] transition-colors leading-tight block max-w-full wrap-break-word"
                      >
                        {link.name}
                      </Link>
                    </li>
        ...
        <div className="flex flex-col lg:flex-row justify-between gap-8 mb-8">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="w-32 h-32 lg:w-20 lg:h-20 bg-white p-2 rounded-xl shadow-sm relative shrink-0">
              {/* 🔥 Используем наш новый API роут для QR кода! */}
              <QRCodeSVG
                value="https://kgloto.com/api/download"
            ...
            <div className="flex flex-col gap-3 lg:hidden">
              ...
              <div className="flex flex-col gap-2">
                <StoreButton
                  type="appstore"
                  href={APP_STORE_LINK}
                  downloadText="Загрузите в"
                />
                <StoreButton
                  type="googleplay"
                  href={GOOGLE_PLAY_LINK}
                  downloadText="Доступно в"
                />
              </div>
            </div>
          </div>

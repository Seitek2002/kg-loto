'use client';

import { Modal } from '@/components/ui/Modal';
import { Smartphone, X } from 'lucide-react';
import { useTranslations } from 'next-intl';

interface AppRedirectModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const AppRedirectModal = ({
  isOpen,
  onClose,
}: AppRedirectModalProps) => {
  const t = useTranslations('modal');

  return (
    <Modal isOpen={isOpen} onClose={onClose} className='max-w-md'>
      <div className='bg-white rounded-4xl p-8 flex flex-col items-center text-center relative overflow-hidden'>
        {/* Кнопка закрытия */}
        <button
          onClick={onClose}
          className='absolute top-5 right-5 text-gray-400 hover:text-gray-800 transition-colors'
        >
          <X size={24} />
        </button>

        {/* Иконка */}
        <div className='w-20 h-20 bg-[#F5F5F5] rounded-full flex items-center justify-center mb-6 text-[#4B4B4B]'>
          <Smartphone size={40} strokeWidth={1.5} />
        </div>

        {/* Текстовка с переводами */}
        <h3 className='text-xl font-black font-benzin uppercase text-[#1C2035] mb-4'>
          {t('title')}
        </h3>
        <p className='text-sm text-[#737373] font-rubik mb-8 leading-relaxed px-2'>
          {t('description')}
        </p>

        {/* Заглушки для сторов с едиными иконками */}
        <div className='flex flex-col sm:flex-row w-full gap-3'>
          <button
            onClick={onClose}
            className='flex-1 flex items-center justify-center gap-2 bg-[#4B4B4B] text-white py-4 rounded-full font-black text-xs uppercase tracking-wider hover:bg-black transition-colors'
          >
            <svg className='w-5 h-5 fill-current' viewBox='0 0 24 24'>
              <path d='M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.5 1.3 0 2.52.87 3.3.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.39 2.82M13 3.5c.73-.83 1.21-1.96 1.07-3.09-1.05.06-2.31.74-3.03 1.59-.65.74-1.23 1.92-1.07 3.07 1.17.09 2.35-.73 3.03-1.57' />
            </svg>
            App Store
          </button>

          <button
            onClick={onClose}
            className='flex-1 flex items-center justify-center gap-2 bg-[#4B4B4B] text-white py-4 rounded-full font-black text-xs uppercase tracking-wider hover:bg-black transition-colors'
          >
            <svg className='w-5 h-5 fill-current' viewBox='0 0 24 24'>
              <path d='M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.5,12.92 20.16,13.19L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z' />
            </svg>
            Google Play
          </button>
        </div>
      </div>
    </Modal>
  );
};

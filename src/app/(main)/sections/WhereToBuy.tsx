...
import { useState, useMemo } from 'react'; // ❌ Убираем useEffect
...
export const WhereToBuy = ({ branches }: WhereToBuyProps) => {
  ...
  // 🔥 2. Стейт хранит ТОЛЬКО ручной выбор пользователя
  const [selectedId, setSelectedId] = useState<string | null>(null);

  // 🔥 3. Вычисляем активный ID "на лету" во время рендера
  // Если пользователь выбрал (selectedId) -> берем его.
  // Если нет -> берем первый филиал из списка.
  const activeId =
    ...
    <section className='my-20 relative'>
      ...
      <div className='relative w-full h-[400px] lg:h-[500px] rounded-[40px] overflow-hidden shadow-sm border border-gray-100'>
        <LeafletMap
          ...
          activeId={activeId}
          onMarkerClick={setSelectedId} // 🔥 При клике обновляем ручной выбор
        />

...
const MOCK_MODAL_DATA = {
  ...
  results: [
    { id: 1, category: '1', guessed: '5', count: '45', win: '-', totalWin: '-' },
    { id: 2, category: '2', guessed: '3', count: '123', win: '-', totalWin: '-' },
    { id: 3, category: '3', guessed: '2', count: '0', win: '-', totalWin: '-' },
    { id: 4, category: '4', guessed: '3', count: '0', win: '56 000 c', totalWin: '56 000 c' },
    { id: 5, category: '5', guessed: '1', count: '5', win: '12 000 c', totalWin: '12 000 c' },
    { id: 6, category: '6', guessed: '5', count: '7', win: '245 c', totalWin: '245 c' },
    { id: 7, category: '7', guessed: '3', count: '43', win: '50 c', totalWin: '50 c' },
  ],
};
...
export const DrawDetailsModal = ({ isOpen, onClose, drawId }: DrawDetailsModalProps) => {
  ...
  const modalContent = (
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 lg:p-0">
      ...
      <div className="relative w-full max-w-[800px] max-h-[90vh] overflow-y-auto bg-[#F5F5F7] rounded-[24px] lg:rounded-[32px] p-4 lg:p-8 shadow-2xl z-10 custom-scrollbar">
        ...
        {/* Заголовок */}
        <h2 className="text-[20px] lg:text-[24px] font-black text-[#2D2D2D] mb-4 lg:mb-6">
          {data.title}
        </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
          ...
          <div className="bg-white rounded-[16px] lg:rounded-[20px] p-5 shadow-sm">
            <h3 className="text-[#2D2D2D] text-[16px] lg:text-[18px] font-bold mb-4">
            ...
            <div className="flex flex-col gap-2 text-[13px] lg:text-[14px]">
              <div className="flex justify-between lg:justify-start lg:gap-2">
                ...
                <span className="text-[#2D2D2D] font-bold">{data.date}</span>
              </div>
              <div className="flex justify-between lg:justify-start lg:gap-2">
                <span className="text-gray-500">Время публикации результатов:</span>
                <span className="text-[#2D2D2D] font-bold">{data.publishTime}</span>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-[16px] lg:rounded-[20px] p-5 shadow-sm mb-4 flex flex-col lg:flex-row lg:items-center justify-between gap-5">
          <div className="flex flex-col gap-2 text-[13px] lg:text-[14px]">
            <div className="flex justify-between lg:justify-start lg:gap-2">
              <span className="text-gray-500">Число билетов, принявших участие в розыгрыше:</span>
              <span className="text-[#2D2D2D] font-bold">{data.totalTickets}</span>
            </div>
            <div className="flex justify-between lg:justify-start lg:gap-2">
              <span className="text-gray-500">Общая сумма выигрышей:</span>
              <span className="text-[#2D2D2D] font-bold">{data.totalWinAmount} <span className="underline">с</span></span>
            </div>
            <div className="flex justify-between lg:justify-start lg:gap-2">
              <span className="text-gray-500">Суперприз:</span>
              <span className="text-[#2D2D2D] font-bold">{data.superPrize} <span className="underline">с</span></span>
            </div>
          </div>
          <button className="bg-[#4B4B4B] hover:bg-[#2D2D2D] text-white py-3 px-6 rounded-full text-[14px] font-bold transition-colors whitespace-nowrap self-start lg:self-center">
            Купить билет
          </button>
        </div>
        ...
        <div className="bg-white rounded-[16px] lg:rounded-[20px] p-5 shadow-sm">
          <h3 className="text-[#2D2D2D] text-[18px] font-bold mb-4">Итоги розыгрыша</h3>
          
          <div className="overflow-x-auto pb-2">
            <table className="w-full text-center text-[13px] lg:text-[14px] min-w-[550px]">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="font-bold text-[#4B4B4B] py-3 text-left">Категория выигрышей</th>
                  <th className="font-bold text-[#4B4B4B] py-3">Угаданных чисел</th>
                  <th className="font-bold text-[#4B4B4B] py-3">Комбинаций</th>
                  <th className="font-bold text-[#4B4B4B] py-3">Выигрыш</th>
                  <th className="font-bold text-[#4B4B4B] py-3 text-right">Сумма выигрышей</th>
                </tr>
              </thead>
              <tbody>
                {data.results.map((row) => (
                  <tr key={row.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                    <td className="py-3 text-[#4B4B4B] text-left">{row.category}</td>
                    ...
                    <td className="py-3 text-[#4B4B4B] text-right">{row.totalWin}</td>
                  </tr>
                ))}
              </tbody>
            </table>
